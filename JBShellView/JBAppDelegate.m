...
- (void)applicationDidFinishLaunching:(NSNotification *)notification {
	...
	NSString *prompt = [NSString stringWithFormat:@"%@> ", @"JBSV"];
   self.shellContainer = [JBShellContainerView.alloc initWithFrame:bounds 
																  	 shellViewClass:nil 
																				prompt:prompt 
													shellInputProcessingHandler:^(NSString *input, JBShellView *sender) {
		[input hasPrefix:kSearchString] ? ^{
		...
		}(): [input hasPrefix:@"poop"] ? ^{
	
			[sender showErrorOutput:@"you farted" errorRange:NSMakeRange(2, 1)];
		}():
		
		[sender appendOutputWithNewlines:@"Try 'search Kubrick'"];
	}];
	_shellContainer.shellView.backgroundColor = NSColor.blackColor;
	_shellContainer.shellView.textColor = NSColor.whiteColor;
